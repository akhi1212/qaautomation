...
### Note :  Assuming VDS-666 scope is to verify only http status 200.
## And  assuming swagger response is correct when it return http status 200.
...
## default url : http://lcoa-dvvd-xap1:8080/vin-description-service/v2/vinDescription
  Scenario: Validate default swagger api is not broken and return http status 200.
    ...
    Then user should see http status code 200 ok and content should be display
